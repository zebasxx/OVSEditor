...
export function createApiFactoryAndRegisterActors(accessor: ServicesAccessor): IExtensionApiFactory {
	...
	const disabledDisposable = Object.freeze({ dispose(): void { } });
	const disabledAIExtensionHostApi = new Proxy(Object.create(null), {
		get(_target, prop) {
			switch (prop) {
				case 'activeChatPanelSessionResource':
					return undefined;
				case 'isModelProxyAvailable':
					return false;
				case 'mcpServerDefinitions':
					return [];
				case 'onDidChangeActiveChatPanelSessionResource':
				case 'onDidDisposeChatSession':
				case 'onDidAddCoreEvent':
				case 'onDidChangeCustomAgents':
				case 'onDidChangeInstructions':
				case 'onDidChangeSkills':
				case 'onDidChangeSlashCommands':
				case 'onDidChangeHooks':
				case 'onDidChangePlugins':
				case 'onDidChangeProviders':
				case 'onDidChangeModelProxyAvailability':
				case 'onDidChangeMcpServerDefinitions':
				case 'onDidChangeChatRequestTools':
					return Event.None;
				case 'selectLanguageModels':
				case 'getRelatedInformation':
				case 'getTools':
				case 'provideCustomAgents':
				case 'provideInstructions':
				case 'provideSkills':
				case 'provideSlashCommands':
				case 'provideHooks':
				case 'providePlugins':
					return async () => [];
				case 'fileIsIgnored':
	...
	const extHostLanguageModels = disabledAIExtensionHostApi;
	const extHostCodeMapper = disabledAIExtensionHostApi;
	const extHostChatOutputRenderer = disabledAIExtensionHostApi;
	const extHostLanguageModelTools = disabledAIExtensionHostApi;
	const extHostChatSessions = disabledAIExtensionHostApi;
	const extHostChatAgents2 = disabledAIExtensionHostApi;
	const extHostChatContext = disabledAIExtensionHostApi;
	const extHostChatDebug = disabledAIExtensionHostApi;
	const extHostAiRelatedInformation = disabledAIExtensionHostApi;
	const extHostAiEmbeddingVector = disabledAIExtensionHostApi;
	const extHostAiSettingsSearch = disabledAIExtensionHostApi;
	const extHostChatStatus = disabledAIExtensionHostApi;
	const extHostChatInputNotification = disabledAIExtensionHostApi;
	const extHostMcp = disabledAIExtensionHostApi;
